...
namespace Accounts.BLL
{
    public class GeneralStockIssuanceHeadBLL
    {
        ...
        public EntityoperationInfo DeleteStockIssuance(Int64 IdVoucher)
        {
            SqlConnection objconn = new SqlConnection(DBHelper.DataConnection);
            try
            {
                objconn.Open();
                return dal.DeleteStockIssuance(IdVoucher, objconn);
            }
            catch (Exception ex)
            {
                objconn.Close();
        ...
        public List<VoucherDetailEL> GetMaterialLocation(Int64 IdItem)
        {
            SqlConnection objconn = new SqlConnection(DBHelper.DataConnection);
            try
            {
                objconn.Open();
                return dal.GetMaterialLocation(IdItem, objconn);
            }
            catch (Exception ex)
            {
                objconn.Close();
        ...
        public List<VoucherDetailEL> GetAllMaterialLocation()
        {
            SqlConnection objconn = new SqlConnection(DBHelper.DataConnection);
            try
            {
                objconn.Open();
                return dal.GetAllMaterialLocation(objconn);
            }
            catch (Exception ex)
            {
                objconn.Close();
