...
namespace Accounts.DAL
{
    public class GeneralStockIssuanceHeadDAL
    {
        ...
        public EntityoperationInfo DeleteStockIssuance(Int64 IdVoucher, SqlConnection objConn)
        {
            EntityoperationInfo infoResult = new EntityoperationInfo();
            using (SqlCommand cmdStore = new SqlCommand("[Transactions].[Proc_DeleteStockIssuance]", objConn))
            {
                cmdStore.CommandType = CommandType.StoredProcedure;
                cmdStore.Parameters.Add(new SqlParameter("@IdVoucher", DbType.Int64)).Value = IdVoucher;

                //if (cmdItems.ExecuteNonQuery() > -1 && cmdAccounts.ExecuteNonQuery() > -1)
                if (cmdStore.ExecuteNonQuery() > -1)
                {
        ...
        public List<VoucherDetailEL> GetGeneralStockIssuance(Int64 IdIssuance, Int64 IdProject, Int64 BookNo, Int32 StoreType, SqlConnection objConn)
        {
            ...
            using (SqlCommand cmdIssuance = new SqlCommand("[Transactions].[Proc_GetGeneralStockIssuance]", objConn))
            {
                ...
                while (objReader.Read())
                {
                    ...
                    oelIssuanceDetail.IdBlock = Validation.GetSafeLong(objReader["Block_Id"]);
                    oelIssuanceDetail.IdLevel = Validation.GetSafeLong(objReader["Level_Id"]);
                    oelIssuanceDetail.IdGrid = Validation.GetSafeLong(objReader["Grid_Id"]);
                    ...
                    oelIssuanceDetail.MIR = Validation.GetSafeBooleanNullable(objReader["MIR"]);
                    oelIssuanceDetail.CurrentUnitPrice = Validation.GetSafeDecimal(objReader["UnitPriceFlat"]);
        ...
        public List<VoucherDetailEL> GetMaterialLocation(Int64 IdItem, SqlConnection objConn)
        {
            List<VoucherDetailEL> list = new List<VoucherDetailEL>();
            using (SqlCommand cmdIssuance = new SqlCommand("[Transactions].[Proc_GetLocationByMaterial]", objConn))
            {
                cmdIssuance.CommandType = CommandType.StoredProcedure;
                cmdIssuance.CommandTimeout = 0;
                cmdIssuance.Parameters.Add(new SqlParameter("@IdItem", DbType.Int64)).Value = IdItem;
                objReader = cmdIssuance.ExecuteReader();
                while (objReader.Read())
                {
                    VoucherDetailEL obj = new VoucherDetailEL();
                    //obj.ItemName = Validation.GetSafeString(objReader["ItemName"]);
                    obj.BlockName = Validation.GetSafeString(objReader["Block_Name"]);
                    obj.LevelName = Validation.GetSafeString(objReader["Level_Name"]);
                    obj.GridName = Validation.GetSafeString(objReader["Grid_Name"]);
        ...
        public List<VoucherDetailEL> GetAllMaterialLocation(SqlConnection objConn)
        {
            List<VoucherDetailEL> list = new List<VoucherDetailEL>();
            using (SqlCommand cmdIssuance = new SqlCommand("[Transactions].[Proc_GetLocationByAllMaterials]", objConn))
            {
                cmdIssuance.CommandType = CommandType.StoredProcedure;
                cmdIssuance.CommandTimeout = 0;
                objReader = cmdIssuance.ExecuteReader();
                while (objReader.Read())
                {
                    VoucherDetailEL obj = new VoucherDetailEL();
                    obj.ItemName = Validation.GetSafeString(objReader["ItemName"]);
                    obj.CategoryName = Validation.GetSafeString(objReader["Category_Name"]);
